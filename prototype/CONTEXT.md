...
## 管理画面通信診断の折りたたみUI 2026-08-24

- `admin/devices`では、QR発行、接続中端末、客席端末とテーブル管理を先に表示し、通信診断は末尾へ移動した。診断は初期状態で1行の「通信状態：正常」サマリーに折りたたみ、`詳細を見る`で既存の技術情報を展開する。
- health異常、認証不一致、safe-copy以外のDB、HTTP 4xx/5xx、診断API取得失敗時だけ自動展開する。注文送信・取得記録がない状態は「再起動後の注文送信記録なし」「再起動後の注文取得記録なし」とし、異常扱いにしない。
