...
## 2026-09-20 タクシー／運転代行データ受入 checkpoint

- A90実機確認ALL PASS。
- タクシー2件、運転代行4件。全6件表示ON、備考空欄。
- 客席表示は「お迎え先　　(居酒屋わるん)」とし、「お迎え先」は赤字、括弧内は黒字。住所は`カカミガハラシ　ナカ　ニッシンチョウ８－３６`。
- event_log最終IDは353、schema v11、safe-copy PIDは16588。
- production DBは未反映。pushは未実施。

## 2026-09-20 タクシー／運転代行案内 checkpoint

タクシー／運転代行案内のschema v11、管理UI、客席UIを実装し、safe-copyで受入状態を確認した。

- safe-copy schema v11。店舗のお迎え先住所・正式な連絡先は未登録のまま（pickup住所空欄、連絡先0件）。
- 管理UI・客席UIを実装済み。A90アプリアイコンでユーザー目視PASSとした。
- タクシー／運転代行の種類選択、一覧、戻る、閉じる、タイトル切替をPASSとした。
- prototype 111/111、server 386/386、Sites 4/4、Vite build PASS。
- event_log 343。注文、商品、営業時間、notice値は不変。
- production DB、pairing、注文、pushは未操作。
- 次工程は正式住所・連絡先登録。
