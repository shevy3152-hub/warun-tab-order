...
function Get-ReadyHealth {
  try {
    ...
    if ($response.StatusCode -eq 200 -and
        ...
        $body.schemaVersion -eq $TargetSchemaVersion -and
...
try {
  ...
  if ($health.StatusCode -ne 200 -or $healthBody.status -ne 'ready' -or $healthBody.db -ne 'ready' -or $healthBody.schemaVersion -ne $TargetSchemaVersion) {
