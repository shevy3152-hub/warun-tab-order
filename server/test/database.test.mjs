...
test('new database creates its parent directory and applies schema v13', async () => {
...
test('new database records PRAGMA user_version = 13', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    const connection = initializeDatabase({ databasePath });
    assert.equal(pragmaValue(connection.database, 'user_version'), 13);
    connection.close();
  });
});
...
test('schema v7 migrates to v8 with every existing menu item in normal mode', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    mkdirSync(dirname(databasePath), { recursive: true });
    const raw = new DatabaseSync(databasePath);
    raw.exec(readFileSync(new URL('../../docs/schema-v2.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v3-migration.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v4-migration.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v5-migration.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v6-migration.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v7-migration.sql', import.meta.url), 'utf8'));
    raw.exec(`
      INSERT INTO categories (category_id, name, sort_order, is_visible, section_key, version, created_at_ms, updated_at_ms)
      VALUES ('migration-category', '移行確認', 1, 1, 'food', 1, 1000, 1000);
      INSERT INTO menu_items (menu_item_id, category_id, formal_name, kitchen_alias, price_yen, is_sold_out, is_active, sort_order, version, created_at_ms, updated_at_ms, section_key)
      VALUES ('migration-item', 'migration-category', '移行商品', '移行商品', 500, 0, 1, 1, 1, 1000, 1000, NULL);
    `);
    raw.close();
    ...
    assert.equal(connection.schemaVersion, 13);
    assert.equal(pragmaValue(connection.database, 'user_version'), 13);
    assert.equal(connection.database.prepare('SELECT schema_version FROM system_state').get().schema_version, 13);
...
test('v8 clone migrates to v9 and rerunning the v9 migration is safe', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    mkdirSync(dirname(databasePath), { recursive: true });
    const raw = new DatabaseSync(databasePath);
    raw.exec(readFileSync(new URL('../../docs/schema-v2.sql', import.meta.url), 'utf8'));
    ...
    raw.exec(readFileSync(new URL('../../docs/schema-v6-migration.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v7-migration.sql', import.meta.url), 'utf8'));
    raw.exec(readFileSync(new URL('../../docs/schema-v8-migration.sql', import.meta.url), 'utf8'));
    raw.close();

    const connection = initializeDatabase({ databasePath });
    const before = connection.database.prepare('SELECT * FROM business_hours WHERE singleton_id = 1').get();
    connection.database.exec(readFileSync(new URL('../../docs/schema-v9-migration.sql', import.meta.url), 'utf8'));
    assert.equal(pragmaValue(connection.database, 'user_version'), 9);
    assert.deepEqual(connection.database.prepare('SELECT * FROM business_hours WHERE singleton_id = 1').get(), before);
    assert.equal(connection.database.prepare('SELECT schema_version FROM system_state').get().schema_version, 9);
    assert.equal(connection.database.prepare('PRAGMA integrity_check').get().integrity_check, 'ok');
    connection.close();
  });
});

test('schema v9 clone migrates through v10, v11, v12, and v13 with notice defaults and no event', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    mkdirSync(dirname(databasePath), { recursive: true });
    const raw = new DatabaseSync(databasePath);
    for (const file of [
      'schema-v2.sql', 'schema-v3-migration.sql', 'schema-v4-migration.sql',
      'schema-v5-migration.sql', 'schema-v6-migration.sql', 'schema-v7-migration.sql',
      'schema-v8-migration.sql', 'schema-v9-migration.sql',
    ]) raw.exec(readFileSync(new URL(`../../docs/${file}`, import.meta.url), 'utf8'));
    raw.close();
...
test('a valid schema v13 database can be closed and reopened', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
...
test('reopening schema v13 preserves existing data', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
...
test('schema v9 repairs the legacy unique pairing-device constraint for revoked re-registration', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
...
test('legacy schema v1 migrates through schema v10 and assigns existing orders to one open table session', async () => {
...
test('schema v4 fixture migrates through the formal v7 path while preserving catalog selections and operational history', async () => {
  await withTemporaryDatabase(({ directory }) => {
    ...
    assert.equal(connection.schemaVersion, 13);
    assert.equal(pragmaValue(database, 'user_version'), 13);
...
test('schema versions newer than v13 are rejected without migration', async () => {
  await withTemporaryDatabase(({ directory }) => {
