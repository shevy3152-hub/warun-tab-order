...
test('new database creates its parent directory and applies schema v12', async () => {
...
test('new database records PRAGMA user_version = 12', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    const connection = initializeDatabase({ databasePath });
    assert.equal(pragmaValue(connection.database, 'user_version'), 12);
    connection.close();
  });
});
...
test('schema v7 migrates to v8 with every existing menu item in normal mode', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    ...
    assert.equal(connection.schemaVersion, 12);
    assert.equal(pragmaValue(connection.database, 'user_version'), 12);
    assert.equal(connection.database.prepare('SELECT schema_version FROM system_state').get().schema_version, 12);
...
test('schema v9 clone migrates through v10, v11, and v12 with notice defaults and no event', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
    mkdirSync(dirname(databasePath), { recursive: true });
    const raw = new DatabaseSync(databasePath);
    for (const file of [
...
test('a valid schema v12 database can be closed and reopened', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
...
test('reopening schema v12 preserves existing data', async () => {
  await withTemporaryDatabase(({ databasePath }) => {
...
test('schema v4 fixture migrates through the formal v7 path while preserving catalog selections and operational history', async () => {
  await withTemporaryDatabase(({ directory }) => {
    ...
    assert.equal(connection.schemaVersion, 12);
    assert.equal(pragmaValue(database, 'user_version'), 12);
...
test('schema versions newer than v12 are rejected without migration', async () => {
  await withTemporaryDatabase(({ directory }) => {
