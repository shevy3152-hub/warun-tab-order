...
test("serves local WebP and PNG assets with image content types", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "sample.png"), Buffer.from("PNG"));
    ...
    const pngResponse = await fetch(`http://127.0.0.1:${port}/sample.png`);
